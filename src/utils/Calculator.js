module.exports = async (message) => {
    if (!message) throw new TypeError("No message object provided.");
    const { MessageButton } = require('./buttons/src/index');
    function i(length) {
        ...
        calc.on('collect', async btn => {
            ...
            if(btn.id === calculator_1){
                ...
                try{
                    ...
                    calc.stop()
                    lock()
                    message.channel.send('Error whilst executing the calculation. Canceling...');
                    str = ''
                    stringify = '```\n' + str + '\n```'
                }
            }
