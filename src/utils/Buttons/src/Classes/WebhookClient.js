...
module.exports = class extends WebhookClient {
  async editMessage(message, content, options = {}) {
    const { data, files } = await (content instanceof APIMessage
      ? content
    ...
    const messageManager = this.client.channels
      ? this.channels.cache.get(d.channel_id)
        ? this.channels.cache.get(d.channel_id).messages
        : null
      : null;
