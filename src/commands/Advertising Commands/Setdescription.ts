...
export const run: RunFunction = async (client, message, args) => {
  ...
  if (!document) {
    return client.utils.sendError(
      `Looks like there was an error finding your Guilds data. Please try again later.`,
      message.channel
    );
  }
  ...
  if (!desc) {
    return client.utils.sendError(
      "You must specify some content for the server description, or supply 'remove'.",
      message.channel
    );
  }
  ...
  if (desc.toLowerCase() === "remove") {
    if (!document.description) {
      return client.utils.sendError(
        "There is no configured description for this server, you must set one before removing it.",
        message.channel
      );
    }
    ...
    return client.utils.succEmbed(
      `Successfully removed your servers description!`,
      message.channel
    );
  }
};
