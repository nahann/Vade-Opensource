...
export const run: RunFunction = async (client, message, args) => {
  ...
  if (!findSchema)
    return client.utils.sendError(`No data found.`, message.channel);
  ...
  if (!type.includes(args[2]?.toLowerCase()))
    return client.utils.sendError(
      `"wallet" or "bank" needs to be specified.`,
      message.channel
    );
  ...
  if (args[2]?.toLowerCase() === "bank") {
    ...
    return client.utils.succEmbed(
      `Successfully completed that action.`,
      message.channel
    );
...
export const description: string =
  "Set a users balance to the amount specified.";
