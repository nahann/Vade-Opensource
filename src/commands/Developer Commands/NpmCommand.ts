...
export const run: RunFunction = async (client, message, args) => {
    ...
    npmsearch(toSearch, {
        ...
        let keywords: string[] = Object.keys(res).includes("keywords") ? (res["keywords"] as string[]).map(keyword => `\`${keyword}\``) : ["No Data"];
        let maintainers: string[] = (res["maintainers"] as string[]).map(maintainer => `\`${maintainer["username"]}\``);
        let links: string[] = Object.keys(res["links"]).map(key => `**${key}：**${res["links"][key]}`);
        ...
        await message.channel.send(
            new client.embed()
                ...
                .addField("Published date", `${publishDate.format("LL LTS")} \`${publishDate.fromNow()}\``)
