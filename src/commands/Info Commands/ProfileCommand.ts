...
import ms from 'ms';
   ...
   export const run: RunFunction = async(client, message, args) => {
    ...
    let Partner = locate_schema?.Partner ? (await client.users.fetch(locate_schema.Partner)).tag : 'No Partner';
    let Job = locate_schema?.Job  ? locate_schema.Job : 'Unemployed';
    let Wage = locate_schema?.Wage ? locate_schema.Wage : 0;
    let HourlyTime = !locate_schema || !locate_schema.HourlyTime || Date.now() > (locate_schema as any)?.HourlyTime ? 'Ready to claim!' : ms(locate_schema?.HourlyTime - Date.now(), { long: true })
    let DailyTime = !locate_schema || !locate_schema.DailyTime || Date.now() > (locate_schema as any)?.DailyTime ? 'Ready to claim!' : ms(locate_schema?.DailyTime - Date.now(), { long: true })
    let WeeklyTime = !locate_schema || !locate_schema.WeeklyTime || Date.now() > (locate_schema as any)?.WeeklyTime ? 'Ready to claim!' : ms(locate_schema?.WeeklyTime - Date.now(), { long: true })
    let MonthlyTime = !locate_schema || !locate_schema.MonthlyTime || Date.now() > (locate_schema as any)?.MonthlyTime ? 'Ready to claim!' : ms(locate_schema?.MonthlyTime - Date.now(), { long: true })
