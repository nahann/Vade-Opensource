import { RunFunction } from '../../interfaces/Command';
   ...
   export const run: RunFunction = async(client, message, args) => {
    if (!args[0]) return client.utils.sendError(`You need to specify either "rock", "paper" or "scissors".`, message.channel);
    ...
    if (args[0].includes('paper') || args[0].includes('rock') || args[0].includes('scissors')) {
      ...
      **I choose:** ${choice}\n
      Result: ${winner} has win`);
        ...
        return client.utils.sendError(`You need to specify either "rock", "paper" or "scissors".`, message.channel);
    }
