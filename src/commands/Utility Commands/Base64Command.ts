...
export const run: RunFunction = async(client, message, args) => {
    ...
    const method = args[0];
    if(!method || method.toLowerCase() !== "encode" && method.toLowerCase() !== "decode") return message.channel.send(`You need to specify either \`encode\` or \`decode\`.`);
    const text = args.slice(1).join(" ");
    if(!text) return message.channel.send(`You need to specify some text to encode/decode.`);
