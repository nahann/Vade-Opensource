...
export const run: RunFunction = async (client, message, args, lang) => {
  ...
  if (!profile || !profile.Wallet || profile.Wallet < bet)
    return client.utils.sendError(
      client.translate({ locale: lang, phrase: "slots.notEnough" }),
      message.channel
    );
  ...
  const endEmbed = new client.embed()
    ...
    .addField(
      client.translate({ locale: lang, phrase: "slots.newBal" }),
      `$${nf.format(profit ? profile.Wallet + profit : profile.Wallet - bet)}`
    )
    .addField("\u200b", `>> ${slots.join("  |  ")} <<`);
  amount === 0
    ? endEmbed.setErrorColor()
    : endEmbed
        .setSuccessColor()
        .setAuthor(message.author.tag, message.author.displayAvatarURL())
        .setTimestamp()
