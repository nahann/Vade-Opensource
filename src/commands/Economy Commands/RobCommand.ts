...
if(!user || user.id === message.author.id || user.id === client.user.id) return client.utils.sendError(`You need to specify who you wish to rob!`, message.channel);
const Res = await mainSchema.findOne({ User: user.id });
const isPassive = Res?.Passive;

if(isPassive) return client.utils.sendError(`That user is in passive mode!`, message.channel);
const authorRes = await mainSchema.findOne({ User: message.author.id });
if(authorRes && authorRes.Passive) return client.utils.sendError(`You cannot rob someone whilst in passive mode!`, message.channel);
const userBal = authorRes.Wallet ?? 0;
if(userBal < 2000) return client.utils.sendError(`You need at least $2,000 Coins to rob someone!`, message.channel);
const robberBal = Res.Wallet ?? 0;
if(robberBal <= 0 || robberBal < 2000) return client.utils.sendError(`You cannot rob someone with less than $2,000 in their Wallet!`, message.channel);
const checkPremium = await client.utils.checkPremium(message.author.id);
const successRate = checkPremium ? Math.floor(Math.random() * 100) : Math.floor(Math.random() * 80) * 1.35;
