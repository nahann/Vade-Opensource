   ...
   export const run: RunFunction = async(client, message, args) => {
    ...
    const Wages = {
      ...
      if(!args.length) {
        ...
        const commandsToPaginate = Jobs.map((j) => `${j}\n Salary: $${nf.format(Wages[j]) ?? `2,000`}\n`);
      ...
      if(economySchema && economySchema.JobSwitchTime > Date.now() + ms('7d')) return message.channel.send(`You can switch jobs again in ${ms(economySchema.JobSwitchTime), { long: true }}`);
      ...
      if(!checked) return message.channel.send(`That is not a valid job. Please run \`!job\` on it's own to view a list of available jobs.`);
      const wage = Wages[client.utils.capitalise(selected)];
      const premCheck = wage >= 15000;
      if(premCheck && !premiumCheck) return message.channel.send(`Jobs with a salary of over 15,000 require you to have Vade Premium!`);
      ...
      if(!economySchema) {
          ...
          return succEmbed(`Successfully set your Job to ${client.utils.capitalise(selected)}!`, message.channel);
      }
