...
import main_schema from '../../../models/economy';
   ...
   export const run: RunFunction = async(client, message, args) => {
    ...
    if(!locate_schema) return message.channel.send(`You don't have any money to give!`);
    const amountToGive = args[1] ? parseInt(args[1]) : 0;

    if(amountToGive <= 0) return message.channel.send(`You need to specify a valid amount to give!`);
    if(!locate_schema.Wallet || locate_schema.Wallet < amountToGive) return message.channel.send(`You don't have enough to give!`);
    ...
    if(!locate_schema_2) {
        ...
        return message.channel.send(`Successfully given ${member.user.tag} **${nf.format(amountToGive)}** and deducted it from your Wallet!`);
    } else {
...
export const description: string = 'Give someone else some money from your Wallet.';
