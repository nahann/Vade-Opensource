...
export const run: RunFunction = async (client, message, args) => {
  ...
  switch (sub) {
      case "removeall": {
        ...
        await Promise.all(member.roles.cache.map(async (role) => {
          try {
            ...
            return message.channel.send(`Unable to remove the role <@&${role}>`, { disableMentions: "all" });
          }
        }));

        return message.channel.send(`Removed all roles from the member successfully.`);
      }
      ...
      case "all": {
        ...
        const role = message.guild.roles.cache.find(({ name }) => name.toLowerCase() === id.toLowerCase()) 
          ...
          try {
              await Promise.all(
                (await Promise.all(ids.map(async (id) => {
                  if (/^(-\+)?\d{18,19}$/.test(id)) return message.guild.roles.fetch(id.match(/^(-\+)?\d{18,19}$/)?.[0] ?? "");
                  ...
                  if (!role) return message.channel.send(`Unable to find the role ${ids[i]}.`, { disableMentions: "all" });

                  try {
                    if (member.roles.cache.has(role.id) || ids[i].startsWith("-")) await member.roles.remove(role);
                    else if (!member.roles.cache.has(role.id) || ids[i].startsWith("+")) await member.roles.add(role); 
                  } catch {
                    return message.channel.send(`Unable to add the role ${ids[i]}.`, { disableMentions: "all" });
                  }
                })
              );
