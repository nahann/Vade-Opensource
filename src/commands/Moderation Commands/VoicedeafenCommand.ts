...
import { GuildMember } from 'discord.js';
   ...
   export const run: RunFunction = async(client, message, args) => {
    ...
    if(member.voice.serverMute) {
        ...
        return client.utils.succEmbed(`Successfully removed the voice deafen for ${member} with the reason: ${reason}.`, message.channel)
    }

   await member.voice.setDeaf(true, reason);

    return client.utils.succEmbed(`Successfully voice deafened ${member} with the reason: ${reason}.`, message.channel);
