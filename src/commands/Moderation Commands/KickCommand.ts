...
export const run: RunFunction = async (client, message, args) => {
    ...
    for (const arg of args) {
        if (/\d{18}/.test(arg)) {
            ...
            if (member && member.id !== client.user?.id && member.id !== message.author.id) {
                if(member.roles.cache && member.roles.highest.position < message.member.roles.highest.position) {
                    members.push(member);
                } else {
                    return message.channel.send(`Could not kick ${member.user.tag} due to them having a higher role than you.`);
                }
        ...
        members.map(async (m, i) => {
            try {
                ...
                if (booleanFlags.has("-S")) {
                    ...
                    client.commands.get("clear")!.run(client, message, ["100", "-u", m.id]);
