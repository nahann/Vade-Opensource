...
message.channel.send("Please specify a channel! **[channel / ID]**\n\nType \`cancel\` to cancel this prompt.").then(() => {
 ...
 if(channel.toLowerCase() === 'cancel'){
    ...
    cancelledEmbed(`Successfully cancelled the prompt!`, message.channel);
    return;
  }
 let channelToSend = client.utils.getChannels(channel, message.guild) as TextChannel;
  
 
    if(!channelToSend) return cancelledEmbed2(`Wrong response! Cancelling the prompt!`, message.channel);;
    ...
    message.channel.send(`Provide me with a message ID\n\nMake sure the message is in ${channelToSend}\n\nType \`cancel\` to cancel this prompt.`).then(() => {
      ...
      return;
        ...
        let messageID = await channelToSend.messages.fetch(ID).catch(() => { return cancelledEmbed2(`Wrong response! Cancelling the prompt!`, message.channel); }) as Message;
      ...
      return;
              ...
              if(!role) return cancelledEmbed2(`Wrong response! Cancelling the prompt!`, message.channel);;
                  if(role.managed){
...
await messageID.react(emoji)
 ...
 return message.channel.send(new client.embed()
    .setAuthor(message.author.tag, message.author.displayAvatarURL())
              ...
              message.channel.awaitMessages(filter, { max: 1, time: 60000, errors: ["time"] }).then(collected4 => {
                ...
                .addField('Role', role, true)
                  ...
                  messageID.react(emoji)
                   ...
                   await client.utils.reactionCreate(message.guild.id , ID, role.id, emoji, false, parseInt(option));//ID is MessageID, ignore "false"

                })

               }).catch(err => { if(err) sendError(`Time has expired! Please run the Command again!`, message.channel); })
              })
               }).catch(err => { if(err) sendError(`Time has expired! Please run the Command again!`, message.channel); })
              })
           }).catch(err => { if(err) sendError(`Time has expired! Please run the Command again!`, message.channel); })
