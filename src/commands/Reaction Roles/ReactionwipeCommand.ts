...
export const run: RunFunction = async (client, message, args) => {
  ...
  if (results && results.length) {
    ...
    return client.utils.succEmbed(
      `Successfully cleared this guilds reaction roles!`,
      message.channel
    );
  } else {
    return client.utils.sendError(
      `Couldn't locate any reaction roles configured for this Server!`,
      message.channel
    );
  }
};
...
export const description: string =
  "Clear all Reaction Roles from the current server.";
