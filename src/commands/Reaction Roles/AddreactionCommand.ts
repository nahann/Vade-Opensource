...
export const run: RunFunction = async (client, message, args) => {
  ...
  let messageID = (await channel.messages.fetch(ID).catch(() => {
    return client.utils.sendError(
      `You must provide a valid message ID.`,
      message.channel
    );
  })) as Message;
  ...
  if (!role)
    return client.utils.sendError(
      `You must provide a valid role. You can provide a role mention, ID or a partial name.`,
      message.channel
    );

  if (role.managed)
    return client.utils.sendError(
      `You cannot use an integrated role.`,
      message.channel
    );
...
export const usage: string =
  "!addreaction <Channel mention> <Message ID> <Role> <Emoji> <Option>";
