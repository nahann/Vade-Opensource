...
export const run: RunFunction = async (client, message, args) => {
  ...
  switch(args[0]?.toLowerCase()) {
    ...
    case "remove": {
      ...
      if(!locate_schema.AdminRole.length) return message.channel.send(`There are currently no Administrator Roles setup for this Server!`);
    ...
    if(!locate_schema.AdminRole.includes(roleID)) return client.utils.sendError(`The Role: ${role} is currently not set as a Administrator role for this Server!`, message.channel);
    ...
    return client.utils.succEmbed(`Successfully removed ${role} from the Administrator Roles in this Server!`, message.channel);
    ...
    if(!role) return message.channel.send(`Please mention a valid role name, ID or mention.`);
    let roleID = role.id;
    if(locate_schema.AdminRole && locate_schema.AdminRole?.length >= 5) return message.channel.send(`You cannot have more than 5 Administrator Roles setup for one Server.`);

    await locate_schema.updateOne({
      $push: { AdminRole: role.id }
    });

    return client.utils.succEmbed(`Successfully set ${role} as a Administrator Role!`, message.channel);
