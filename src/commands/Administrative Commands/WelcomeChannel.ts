...
export const run: RunFunction = async (client, message, args) => {
  ...
  if (!channel && args[0].toLowerCase() !== "remove")
    return message.channel.send(
      `Please state a channel name, ID or mention within this server. Or state "remove".`
    );
  ...
  if (channel && args[0].toLowerCase() !== "remove") {
    if (check_data) {
      ...
      return message.channel.send(
        `Successfully updated your configured welcome channel.`
      );
    }
    ...
    await newData.save();
    return message.channel.send(
      `Successfully set your configured welcome channel.`
    );
  }
};
