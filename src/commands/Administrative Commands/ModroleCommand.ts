...
export const run: RunFunction = async (client, message, args) => {
  ...
  switch (args[0]?.toLowerCase()) {
    ...
    case "remove": {
      if (!locate_schema.ModRole.length)
        return message.channel.send(
          `There are currently no Moderator Roles setup for this Server!`
        );
      ...
      if (!locate_schema.ModRole.includes(roleID))
        return client.utils.sendError(
          `The Role: ${role} is currently not set as a Moderator role for this Server!`,
          message.channel
        );
      ...
      return client.utils.succEmbed(
        `Successfully removed ${role} from the Moderator Roles in this Server!`,
        message.channel
      );
    }
    ...
    case "add": {
      ...
      if (locate_schema.ModRole && locate_schema.ModRole?.length >= 5)
        return message.channel.send(
          `You cannot have more than 5 Moderator Roles setup for one Server.`
        );
      ...
      return client.utils.succEmbed(
        `Successfully set ${role} as a Moderator Role!`,
        message.channel
      );
    }
