...
export const run: RunFunction = async (client, message, args) => {
  ...
  if (
    ...
    return client.utils.sendError(
      `You need to provide a valid game. For example: !linkgame @role "Valorant". Valid games:\n ${client.config.validGames.join(
        "\n"
      )}`,
      message.channel
    );
  ...
  if (LocateAll && LocateAll.length >= 3 && !checkPremium)
    return message.channel.send(
      `The guild owner must own Vade Premium to link more than 3 games.`
    );
  if (!args[0])
    return client.utils.sendError(
      `You need to provide a valid role or specify "remove!.`,
      message.channel
    );
  if (args[0].toLowerCase() === "remove") {
    if (!args[1])
      return client.utils.sendError(
        `You need to provide a role name, mention or ID.`,
        message.channel
      );
    ...
    if (!checkValid)
      return client.utils.sendError(
        `That role is not linked to a game.`,
        message.channel
      );
    await checkValid.delete();

    return client.utils.succEmbed(
      `Successfully removed that binding.`,
      message.channel
    );
  }
  ...
  const LocatedSchema = await MainSchema.findOne({
  ...
  if (!LocatedSchema) {
    ...
    return client.utils.succEmbed(
      `Successfuly linked ${client.utils.capitalise(
        regex[0]
      )} to the role ${role}!`,
      message.channel
    );
    ...
    return client.utils.sendError(
      `That role is already linked to another game! It is linked to "${LocatedSchema.gameName}`,
      message.channel
    );
  }
...
export const description: string =
  "Link a game to a Discord Role. When someone begins to play the game, they will be given the specified role.";
