   ...
   export const run: RunFunction = async(client, message, args) => {
    ...
    if (!args.length) {
        ...
        let embed = new client.embed()
            ...
            .setDescription(
                `Blacklisted Roles: ${roleList.length ? roleList.join(' ') : '**None**'}`
            )
            .setFooter(fetchList.enabled ? 'Enabled' : 'Disabled', message.guild.iconURL())
            .setTimestamp();
    ...
    switch (args[0].toLowerCase()) {
        case 'toggle':
            if (!fetchList) {
                ...
                succEmbed(`Sticky role has been **ENABLED** on the server!`, message.channel);
            } else {
                try {
            ...
            break;
        case 'blacklist':
            if (!roleID) return sendError('Please mention a valid role', message.channel);
            if (newList)
                return sendError(
...
export const description: string = 'Enable sticky roles on this Server. Anyone that leaves, their roles will be remember for the next time they join. Blacklisted roles will not be added.';
