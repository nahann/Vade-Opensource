...
export const run: RunFunction = async (client, message, args) => {
  ...
  await msgs.forEach(async (msg: Message) => {
    ...
    pObj.addText(
      `  ❤️  ${msg.createdAt.toDateString()}  ❤️  ${msg.createdAt.toLocaleTimeString()}`,
      { font_face: "Arial", color: "3c5c63", bold: true, font_size: 14 }
    ); //
    //LINEBREAK
    pObj.addLineBreak();
    //message of user
  ...
  out.on("finish", function (err, result) {
    try {
      ...
      message.channel.send(attachment).then((del) => {
        //after sending it delete the file and edit the temp message to an approvement
        temporarymsg.edit(
          new client.embed().setAuthor(
      ...
      temporarymsg.edit(
        new client.embed().setTitle(`Too big!`).setErrorColor()
      );
