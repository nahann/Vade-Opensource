...
import mainUser from '../../../models/economy';
   ...
   export const run: RunFunction = async(client, message, args) => {
    ...
    if(!schemaLocate) return client.utils.sendError(`You think you're rich enough to marry someone? Get a job and try again when you're financially stable.`, message.channel);

    const married = schemaLocate ? schemaLocate.Partner : null;
    if(married) return client.utils.sendError(`You are already married to ${(await client.users.fetch(schemaLocate.Partner))?.tag}`, message.channel);
    ...
    if(!partnerSchema) {
        ...
        return client.utils.succEmbed(`You're now married to ${message.member}!`, message.channel);
    }
