...
//Track start
    client.manager.on('trackStart', async (player, track) => {
        const channel = await client.channels.fetch(player.textChannel) as TextChannel;
      ...
      track.requester?.id  ? track.requester : `<@${track.requester}>`
            ...
            .setFooter(
                `🎤 ${track.author}  •  ${
                    track.isStream ? `◉ LIVE` : `🕒 ${client.utils.msConversion(track.duration)}`
                }`
            );
        ...
        collector.on('collect', (reaction, user) => {
            ...
            let vchannel = guild.members.cache.get(user.id).voice.channel as VoiceChannel;
